...
def show_favorites_tab():
    ...
    with col_batch_ops[2]:
        if st.button("💰 试算", key="batch_calculate"):
            selected_indices = st.session_state.selected_favorites
...
def show_calculation_config_window(selected_products):
    ...
    selected_store = st.radio("", store_options, key="store_selection")
    ...
    for option in store_config['options']:
        ...
        with col1:
            selected = st.checkbox(option['name'], key=f"discount_{option['name']}")
        with col2:
    ...
    # 一键试算按钮
    if st.button("🚀 一键试算", key="calculate_final"):
...
def display_calculation_results(selected_products, result):
    ...
    with col2:
        ...
        if result['gift_coupon'] > 0:
            ...
            st.metric("商品券优惠", "0韩元")
            st.metric("最终实付", f"{result['final_payment']:,.0f}韩元/")
    # 显示国内总价和折扣信息
    st.divider()
    st.write("**国内价格对比:**")
